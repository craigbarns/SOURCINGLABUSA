...
The largest opportunities were decision-support depth, contextual discovery of the resource hub, reusable original tools, a more explicit method, useful lead qualification and external proof. The implementation adds **10 pages**, expands the existing resource hub and two guides, adds **four downloadable worksheets**, and connects commercial pages to supplier and quality evidence. The owner-requested release is **live**, merged through [PR #5](https://github.com/craigbarns/SOURCINGLABUSA/pull/5) at `dfd364d`, with Netlify deployment `6aa819ea09b878000868d7dd` confirmed ready. The public crawl and HTML audit pass across **31 pages, 1,198 internal links and five downloads**. All 21 former public sitemap URLs and the crawler policy are preserved. See [release evidence](audits/2026-09-14-release.json), [production crawl](audits/2026-09-14-production.json) and [production verification](audits/2026-09-14-production-verification.json).
...
| MEDIUM IMPACT | The pre-existing `/app` route redirects to `app.sourcinglabusa.com`, which returns DNS NXDOMAIN from both the local resolver and Google Public DNS. | The separate application destination is unavailable. The public marketing sitemap and conversion forms are unaffected. | Configure the intended application DNS/hosting through the domain owner and verify HTTPS/authentication before advertising it. No DNS or app exposure changes were made as part of this marketing release. |
...
- **Crawler access:** existing wildcard policy allows public pages, excludes `/app` and `/api/`. Baseline and post-release HTTP probes using Googlebot, Bingbot, OAI-SearchBot, ChatGPT-User, GPTBot, PerplexityBot and Claude-SearchBot user-agent strings returned 200 with content and no noindex header. These are user-agent probes from the audit environment, not requests from verified crawler IPs. Netlify project and deployment metadata were inspected; full CDN/firewall policy exports and verified bot logs were not obtained. Preview noindex is verified on Netlify. App noindex passes in source/local tests, but the public app destination cannot be checked because its DNS is missing.
...
- Production build succeeds with `npm run build -- --webpack`. The default local Turbopack attempt could not bind its internal port in the restricted environment; the project build configuration was not changed. The unchanged default build, tests, lint, types and dependency audit all passed in [GitHub CI for the released revision](https://github.com/craigbarns/SOURCINGLABUSA/actions/runs/34865458282).
...
- **Deployed homepage Lighthouse:** performance **95 mobile / 100 desktop**; accessibility, best practices and Lighthouse's limited SEO checks **100 in both**. Mobile simulated LCP 1.91 seconds, TBT 78 ms, CLS 0; desktop LCP 0.55 seconds, TBT 0 ms, CLS 0.000084. These are single lab navigations against the real CDN and production scripts, not field CWV, INP or site-wide SEO scores. [Configuration, timestamps and results](audits/2026-09-14-lighthouse-production.json) are retained separately from local results.
- **Spanish follow-up requested by the owner:** the longer headline wrapped into four large lines at common desktop/tablet widths. Its font now scales to the Spanish text column; `EE. UU.` stays together. Six widths from 320 to 1,440 pixels were checked: two lines from 390 upward, three on the narrowest viewport, no horizontal overflow, and unchanged English headline sizing. See [observations](audits/2026-09-14-spanish.json), [desktop capture](audits/2026-09-14-spanish-1440.png) and [mobile capture](audits/2026-09-14-spanish-390.png). This follow-up leaves the language URLs and business claims intact.
...
| --- | --- | --- |
| Days 1–3 | Complete the pending legal/business identity register | Business owner: exact entity names, approved founder/profile evidence and category limits |
| Completed September 14 | Release through the existing deployment pipeline and run the same crawl on production | PR #5 merged; 31 public pages verified; all 21 old sitemap URLs retained |
| Days 1–7 | Resolve the separately configured application subdomain if that application is intended to be available | Domain/application owner: DNS, HTTPS, authentication and noindex verification; marketing forms do not depend on it |
